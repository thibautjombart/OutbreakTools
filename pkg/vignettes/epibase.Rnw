...
\title{Handling disease outbreak data using \textit{epibase} \Sexpr{packageDescription("epibase", fields = "Version")}}
...
\begin{abstract}
  This vignette introduces the main functionalities of \textit{epibase}, a package implementing core
  tools for the analysis of outbreak data.  Disease outbreak data can be diverse and complex, and
  the purpose of \textit{epibase} is to simplify the handling of this information.  The main feature
...
\textit{epibase}, and then explain how to import data into the package.
...
Data collected during outbreaks can be hugely diverse and complex.  In \textit{epibase}, our purpose
...
% % % % % % % % % % % % % % % % %
The main class of objects in \textit{epibase} is \texttt{obkData}.  This formal (S4) class is used
...
\item \texttt{@samples}: a \texttt{data.frame} storing sample data, typically swab results or
  ...
  \texttt{sampleID} (identifiers for samples, possibly repeated if several measurements have been
  made on the same sample), and \texttt{date} (collection dates for the samples).
...
samples (\texttt{@samples}), clinical events (\texttt{@clinical}), some DNA sequences
(\texttt{@dna}), a contact network (\texttt{@contacts}) and a phylgenetic tree (\texttt{@trees}).
...
Pathogen sequence data can typically be obtained for different genes, making the handling of such information not entirely trivial.
...
In \texttt{obkData} objects, sequences are matched to samples by the field \texttt{sequenceID} in
the \texttt{@sample} slot, which effectively contains the sequence identifiers.
When several loci have been sequenced, the locus information must also be provided for each
sequence identifiers using the column \texttt{locus} in the \texttt{@sample} slot.
...
\item read data into R
  ...
  \item read DNA sequences from a single file, typically using \texttt{read.dna} from the ape
    ...
    identifiers for the sequences as labels. While phylogenies can be obtained in R, annotated trees
...
\begin{itemize}
  \item \texttt{individuals}: a \texttt{data.frame} with a mandatory column named 'individualID', providing unique identifiers for the individuals.
    \item \texttt{samples}: a \texttt{data.frame} with 3 mandatory columns named 'individualID',
      'sampleID', and 'date', providing identifiers for the individuals, for the samples, and
      dates. Dates must be provided in a way convertible to \texttt{Date} (see
      \texttt{?as.Date}). Default format for dates provided as characters is "\%Y-\%m-\%d"
      (e.g. 1984-09-23). Alternative format can be specified via the argument \texttt{date.format}.
      If left to \texttt{NULL}, the format is determined automatically.
    ...
    \item \texttt{dna}: a list of DNA sequences in \texttt{DNAbin} or \texttt{character} format, as
      read by \texttt{read.dna} or \texttt{fasta2DNAbin}.
    \item \texttt{contacts}: a matrix of characters indicating edges using two columns; if contacts
      ...
      IDs (as returned by \texttt{get.individuals(x)}); if numeric values are provided, these are converted
      to integers and assumed to correspond to individuals returned by \texttt{get.individuals(x)}.
    ...
    \item \texttt{contacts.duration}: another way to specify \texttt{contacts.end}, as duration of contact in
      days.
    \item \texttt{contacts.directed}: a logical indicating if contacts are
      directed; defaults to \texttt{FALSE}.
    \item \texttt{trees}: a list of phylogenetic trees in the class \texttt{multiPhylo} (from the
      ape package); this is basically a list of \texttt{phylo} objects, with the class attribute ``\texttt{multiPhylo}''.
    \item \texttt{date.format}: a character string indicating the date format (see \texttt{as.Date}); if \texttt{NULL}, date format is detected automatically.
\end{itemize}
...
head(x)
@
This apparently did not do much, as the stored information is nearly identical to the input.
However, a number of checks have been made to ensure that dates are actually \texttt{Date} objects,
that individual identifiers of are indeed unique, etc.
More importantly, the two sources of information are now connected, so that we can later e.g. subset the
data per sample or per individual.
This is possible for many other types of information we may want to handle.
Let us now consider the following, additional inputs, including information about a dynamic contact network:
<<>>=
...
\begin{itemize}
  \item individual labels are unique and consistent across samples, clinical interventions, contacts, and
    patient information
  \item every item is dated using actual dates (\texttt{Date} objects), using the same format
  \item every sample refers to an individual, for which meta-information may be available
  \item every DNA sequence refers to a sample
...
Moreover, as all these items are now connected, data manipulation on every components will now be
drastically simplified (see section below on data handling).
...
In the following, we illustrate accessors using a random sample (n=5) of the toy dataset \texttt{ToyOutbreak}:
<<>>=
data(ToyOutbreak)
set.seed(1)
...
\begin{itemize}
\item \texttt{get.individuals(x, data=[\textit{"all" or "samples" or "individuals" or "clinical" or "contacts"}])}: returns the
  individual IDs in different components of the object.

\item  \texttt{get.nindividuals(x, data=[\textit{"all" or "samples" or "individuals" or "clinical"
      or "contacts"}])}: returns the number of individuals in different components of the object.
<<>>=
get.nindividuals(x)
...
@
as there are dates in both \texttt{@samples} and \texttt{@clinical}.
...
\texttt{get.[\textit{...}]}.
For instance (using a random subset of \texttt{ToyOutbreak}):
<<subset1>>=
data(ToyOutbreak)
set.seed(1)
get.nsamples(ToyOutbreak)
...
get.individuals(x)
@
To retain only individuals \Sexpr{get.individuals(x)[3]} and \Sexpr{get.individuals(x)[5]}, once can do:
<<>>=
...
This is particularly useful for instance for select specific test outcomes (e.g. positive swabs), patients within
a given age class or of a given sex, or data from a given location.
For instance, we can retain data from Mexico using:
<<>>=
...
In \textit{epibase}, the function \texttt{make.phylo} is a wrapper for these methods, allowing to
...
parameters distance, and we may want to display the tree and indicate collection dates using tip colors;
this can be done by:
...
Finally, note that \textit{epibase} also integrates functions to read annotated trees with Newick
...
\textit{epibase} provides some basic functionality for the simulation of outbreak data through the \texttt{simuEpi} function.
...
These are a lot of horses in one plot, and we may want to restrict the plot to a selection of individuals.
We can do this by a vector specifying the indices of the individuals to plot. Lets plot the first twenty:
<<plotfirst20>>=
plot(HorseFlu,selection=1:20)
@

\noindent Notice that the names of the individuals are now plotted. The default behaviour is to plot these when less than fifty individuals are plotted, but we can manually override this by setting \texttt{plotNames}.
The plotting of sampling times is toggled by \texttt{plotSamples}. This defaults to TRUE, as an error will be generated when no 'date' fields can be found to plot, as would be the case for the equine dataset.
\\

We can also colour individuals by a characteristic provided in the \texttt{obkData} object.
In this case, let us colour the horses by the yard they were in, a column called 'yardID'
<<>>=
plot(HorseFlu,selection=1:20,colorBy='yardID')
@

\noindent It might be useful to also order the individuals, which can be done according to some
provided information (here, the yard) using \texttt{orderBy}:
<<>>=
