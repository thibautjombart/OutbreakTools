...
subset(x, individuals=NULL, samples=NULL, date.from=NULL, date.to=NULL,
       date.format="", locus=NULL, sequences=NULL, ...)
@
...
@
Note that several filters can be specified at the same time.
For instance, we can retain only the first individual and data of the second gene:
<<>>=
subset(x, indiv=1, locus=2)
@
