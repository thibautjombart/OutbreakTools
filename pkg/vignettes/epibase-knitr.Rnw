...
library(knitr)
opts_chunk$set(fig.path='figs/epibase-', fig.keep='high', dev='pdf', fig.width=7, fig.height=7,
               tidy=FALSE, warning=FALSE, fig.show='asis', fig.align='center')
@
...
For instance, using a graph derived from the first 7 individuals in \texttt{ToyOutbreak}:
<<out.width==".7\\textwidth">>=
x <- subset(ToyOutbreak, individuals=1:7)
get.ncontacts(x)
...
\textit{epibase} provides basic functionality for the simulation of outbreak data through the \texttt{simuEpi} function.
...
In \textit{epibase}, the function \texttt{get.phylo} is a wrapper for these methods, allowing to
