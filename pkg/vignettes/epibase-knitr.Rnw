...
outbreak, without the user worrying about storage issues and consistency amongst different types of
...
\begin{itemize}
  \item \texttt{individuals}: a \texttt{data.frame} with a mandatory column named 'individualID', providing unique identifiers for the individuals.
    ...
    \item \texttt{contacts.duration}: another way to specify contacts.end, as duration of contact in
      days.
...
\begin{itemize}
  ...
  \item every contact refers to a documented individual
\end{itemize}
...
get.ncontacts(x)
plot(x@contacts, main="Contacts in x")
...
a given age class, of a given sex, or data from a given location.
...
For instance, in the following we extract European data collected between 1st June and the 31st August:
<<lastsubset>>=
...
\item `\texttt{geo}' plots the cases on a map. Needs geographical information
\item `\texttt{mst}': plots a minimal spanning tree of the genetic data
\item `\texttt{phylo}': plots a phylogenetic tree of the genetic data
\item `\texttt{contacts}': plots a phylogenetic tree of the genetic data
\end{itemize}
...
<<dev='png'>>=
plot(ToyOutbreak,'geo',location=c('lon','lat'),isLonLat=TRUE,zoom=15,colorBy='Sex',center='11')
@



%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Buidling minimum spanning trees of the genetic sequences}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
data(HorseFlu)
@
<<plotmst1, cache=TRUE, fig.keep="last", out.width=".8\\linewidth">>=
plot(HorseFlu,'mst')
@
