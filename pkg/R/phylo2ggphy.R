...
plot_ggphy<-function(ggphy,tip_attribute=NULL,var_tip_label=NULL){
	...
	if(!is.null(tip_attribute) & !is.null(var_tip_label)){
	#merge df_tip with tip attributes
	tmp<-merge(df_tip,tip_attribute,by.x="label",by.y=var_tip_label)
	tmp$is_FF100<-tmp$pid
	tmp$is_FF100[!is.na(tmp$is_FF100)]<-TRUE
	df_tip<-tmp
	}
	
	#TODO: merge df_edge and df_node with edge and node attributes
	
		
	#df_edge$is_FF100[is.na(df_edge$is_FF100)]<-FALSE	
	...
	#CONF
	p<-ggplot(df_edge)
	p<-p+geom_point(data=df_tip[!is.na(df_tip$is_FF100),],aes(x=x,y=y),colour="red",alpha=0.75,size=5)
	p<-p+geom_segment2(data=df_edge,aes(x=x_beg,xend=x_end,y=y_beg,yend=y_end,alpha=factor(conf)))
	p<-p+scale_alpha_manual("bootstrap",breaks=c(50,100),values=c(0.3,1),labels=c("< 70","> 70"))
	p<-p+guides(colour=guide_legend(override.aes=list(alpha=1)))
	#print(p)
	gt <- ggplot_gtable(ggplot_build(p)) 
	#cairo_pdf(paste(dir_pdf,"FF100_fractal_bootstrap.pdf",sep="/"),height=8,width=6)
	cairo_pdf(paste(dir_pdf,"FF100_fractal_bootstrap.pdf",sep="/"),height=4,width=7)
    grid.draw(gt)
   	dev.off()

	#imported vs UK
	p<-ggplot()
	p<-p+geom_point(data=df_tip[!is.na(df_tip$is_FF100),],aes(x=x,y=y,color=is_imported),alpha=0.95,size=5)
	p<-p+geom_segment2(data=df_edge,aes(x=x_beg,xend=x_end,y=y_beg,yend=y_end,alpha=factor(conf)))
	p<-p+scale_alpha_manual("bootstrap",breaks=c(50,100),values=c(0.3,1),labels=c("< 70","> 70"))
	p<-p+guides(colour=guide_legend(override.aes=list(alpha=1)))
	#print(p)
	gt <- ggplot_gtable(ggplot_build(p)) 
#	cairo_pdf(paste(dir_pdf,"FF100_fractal_infRoute.pdf",sep="/"),height=4,width=7)
 	cairo_pdf(paste(dir_pdf,"FF100_fractal_infRoute.pdf",sep="/"),height=8,width=6)
    grid.draw(gt)
   	dev.off()

	#region
	p<-ggplot()
	p<-p+geom_point(data=df_tip[!is.na(df_tip$is_FF100),],aes(x=x,y=y,color=ff100_region),alpha=0.5,size=5)
	p<-p+geom_segment2(data=df_edge,aes(x=x_beg,xend=x_end,y=y_beg,yend=y_end,alpha=factor(conf)))
	p<-p+scale_colour_brewer("UK region",palette="Spectral")
	p<-p+scale_alpha_manual("bootstrap",breaks=c(50,100),values=c(0.3,1),labels=c("< 70","> 70"))
	p<-p+guides(colour=guide_legend(override.aes=list(alpha=1)))
	#print(p)
	gt <- ggplot_gtable(ggplot_build(p)) 
	#cairo_pdf(paste(dir_pdf,"FF100_fractal_region.pdf",sep="/"),height=8,width=7)
	cairo_pdf(paste(dir_pdf,"FF100_fractal_region.pdf",sep="/"),height=4,width=7)
    grid.draw(gt)
   	dev.off()
   	
	#school
	keep_school<-c("Welford","Alleyn","Eton","Hampton","Lady Eleanor Holles","Barnes")
	keep_school<-c("Alleyn","Eton","Hampton","Lady Eleanor Holles")

	df<-df_tip[df_tip$sch_cd%in%keep_school,]
	df$sch_cd<-factor(df$sch_cd,levels=keep_school)
	p<-ggplot()
	p<-p+geom_point(data=df,aes(x=x,y=y,color=sch_cd),alpha=0.5,size=5)
	p<-p+geom_segment2(data=df_edge,aes(x=x_beg,xend=x_end,y=y_beg,yend=y_end,alpha=factor(conf)))
	p<-p+scale_colour_brewer("School",palette="Spectral")
	p<-p+scale_alpha_manual("bootstrap",breaks=c(50,100),values=c(0.3,1),labels=c("< 70","> 70"))
	p<-p+guides(colour=guide_legend(override.aes=list(alpha=1)))
	print(p)
	gt <- ggplot_gtable(ggplot_build(p)) 
	cairo_pdf(paste(dir_pdf,"FF100_fractal_school.pdf",sep="/"),height=8,width=7)
#	cairo_pdf(paste(dir_pdf,"FF100_fractal_school.pdf",sep="/"),height=4,width=8)
    grid.draw(gt)
   	dev.off()
...
dir<-"/Users/tonton/Documents/Project/Phylogeny/sequence/H3N2/results/BEAST/results/skyline/H3_WD_n1000_GTRGI_E_skyline"
...
#create a phylo
	...
	my_ggphy<-phylo2ggphy(phylo,tip_dates=NULL,branch_unit="year")
	#PLOT_PHYLO(df_list)
